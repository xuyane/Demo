  ...
  <Row class='varietybox'>
    ...
    <Form ref="form" :model="form" :label-width="120">
      <Row type="flex" justify="space-between">
        <Col span='7'>
        <FormItem label="标准名称:" :label-width="100">
...
import {
  getBreedStandardList,
  getBreedStandardAdd,
  getBreedStandardDetail,
  getBreedStandardEdit,
  getBreedStandardRepeat
...
export default {
  ...
  data() {
    const validatePass = (rule, value, callback) => {
      let params = {
        breedStandardId:this.tid,
        name: value
      };
      getBreedStandardRepeat(formData(params)).then(res => {
  ...
  methods: {
    ...
    handleAddSubmit() {
      ...
      this.loading1 = true;
         this.$refs.addFormValidate.validate(valid => {         
          if (valid) {
            getBreedStandardAdd(that.addForm).then(res => {
    ...
    editMsg(id) {
      ...
      getBreedStandardDetail(formData(params)).then(res => {
    ...
    handleEditSubmit() {
      ...
      that.$refs.updateFormValidate.validate(valid => {
        if (valid) {
          getBreedStandardEdit(that.updateForm).then(res => {
    ...
    onOrOffConfig(id, type) {
      ...
      if (type == 0) {
        this.msgContent = "是否确认要禁用该标准?";
      } else if (type == 1) {
        this.msgContent = "是否确认要启用该标准?";
      } else {
        this.msgContent = "类型错误?";
        this.$refs.showIsUseModel.showModal = false;
      }
      this.enableUrl = "/basedata/breedStandard/enable";
      this.disableUrl = "/basedata/breedStandard/disable";
      this.cid = id;
      this.cType = type;
    },
    ...
    queryData() {
      ...
      getBreedStandardList(params).then(res => {
