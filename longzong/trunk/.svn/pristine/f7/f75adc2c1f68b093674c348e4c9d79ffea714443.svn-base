...
import { adminCall,exportAdminCall} from "../../components/axios/produceReport.js";
...
export default {
  ...
  methods: {
    ...
    this.$Spin.show();
		adminCall(params).then(res => {
      that.total =  res.response.callAdminVOs.total;
			that.tableData = res.response.callAdminVOs.list;
    ...
    exportReport(){
      ...
      this.$Spin.show();
      exportAdminCall(params).then(res=>{ 
        if(res.status==200){
