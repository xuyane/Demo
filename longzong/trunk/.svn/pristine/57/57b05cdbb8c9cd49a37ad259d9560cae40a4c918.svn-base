    ...
    <Row class="my-saleList-out">
        ...
        <Form ref="addForm" :label-width="100" class="my-account-conter">
            <Row>
                <Col span="8">
                <FormItem label="认账时间:">
                    <DatePicker type="daterange" v-model="dateRange" class="search-input" placeholder="选择时间段"></DatePicker>
                </FormItem>
                </Col>
                <Col span="8">
                <FormItem label="部门名称:">
        ...
        <Row class-name="table-con">
            ...
            总金额: <span class="font-c-blue mar-r-10">{{formatPrice(amountStatistics.totalAmount)}}</span>
...
import {
  efaccounttransferAdminList,
  efaccounttransferAdminListTotal,
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          minWidth: 160,
          render: (h, data) => {
            return h("span", {}, formatThousandPrice(data.row.totalAmount));
          }
        },
        {
          key: "newAmount",
...
//    this.initDateRange();
  ...
  methods: {
    ...
    getList() {
      ...
      this.loading1 = true;
      efaccounttransferAdminList(param).then(res => {
        this.loading1 = false;
        if (res.status == 200) {
      ...
      efaccounttransferAdminListTotal(param).then(res => {
        if (res.status == 200) {
    ...
    exportExcle() {
      if (this.pageTotal >= 10000) {
        ...
        param.deptIdList = this.params.deptIdList
          ...
          : null;
        axios({
          url: "/report/efaccounttransfer/admin_export",
          method: "get",
          params: param
        }).then(res => {
          if (res.data.status === "500" || res.data.status === "100053") {
            ...
            window.location.href =
              "/report/efaccounttransfer/admin_export?" + form_data;
          }
        });
      }
    },
