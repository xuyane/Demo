    ...
    <Col span="24" class="grid-table">
      <Card>
        <Row class="tablebox">
          ...
          <table width="100%" border="0" cellspacing="0" class="tablewrap">
            <Row>
              ...
              <Col span="12" class="row" >
                <th class="th ">合同状态：</th>
                <td class="td blueword">{{refundAgreement.auditStatus}}</td>
              </Col>
              <Col span="12" class="row">
                <th class="th">业务员</th>
                <td class="td blueword">{{refundAgreement.creatorName}}</td>
              </Col>
            </Row>
          </table>
        </Row>
        <Row class="tablebox">
          ...
          <table width="100%" border="0" cellspacing="0" class="tablewrap">
            <Row>
              <Col span="12" class="row" >
                <th class="th">甲方：</th>
                <td class="td blueword">{{agreement.partA}}</td>
              </Col>
              <Col span="12" class="row">
                <th class="th">乙方：</th>
                <td class="td blueword">{{agreement.partB}}</td>
              </Col>
              <Col span="12" class="row" >
                <th class="th ">用户名：</th>
                <td class="td blueword">{{agreement.userName}}</td>
              </Col>
              <Col span="12" class="row">
        ...
        <Row class="actionwrap">
            ...
            <div v-else>
              <!-- <Button type="primary" class="search-btn">上传合同</Button>
              <Button type="primary" class="search-btn" style="margin-left: 8px">认账</Button> -->
              <Button type="primary" class="search-btn" @click="passagree(1)">通过</Button>

              <Button type="error" class="search-btn" @click="passagree(0)" style="margin-left: 8px">驳回</Button>
    ...
    return {
      ...
      columns1: [
        ...
        // },
        {
          ...
          render: (h, data) => {
            if (
              ...
              return h("div", "服务期限"+data.row.number+this.gettimeunit(data.row.period));
            } else if (
              ...
              return h("div", a+data.row.number+this.gettimeunit(data.row.period));
               ...
               data.row.productCategoryCode.slice(0, 3) == "002" 
            ) {
              var telspan = "";
        ...
        // },
        {
          ...
          render: (h, data) => {
            return h("div", [
              h("p", data.row.subtotalStandardPrice +this.getunit(data.row.unit))
              // h(
              //   "p",
              //   {
              //     style: {
  ...
  methods: {
    ...
    passagree(data){
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          that.spinShow=true;
          contractChangeapplyInfoContractApplyAudit(formData(msg)).then(resp => {
            that.spinShow=false;
    ...
    getMsg() {
      ...
      contractChangeapplyInfoAll(params).then(resp => {
        ...
        that.producelist = resp.response.contractChangeApplyVO.contractChangeProductVOs;
    ...
    getProduct() {
      ...
      getProductInfo(data).then(resp => {
        ...
        that.contractDescription=resp.response.contractDescription||'略'
