            ...
            <Row :gutter="20">
                <Col span='24' >
                  ...
                  <Card class="tablewrap">
                    <Row style="margin-bottom:10px"><span>{{allocationTime}} 待续入合同自动分配结果     分配用户合同总数： {{totalmsg.total}}  &nbsp;&nbsp;     已分配数：{{totalmsg.haduserNum}}</span> </Row>
                    <Row >
                    ...
                    <Row type="flex" justify="center"><Col  ><Button type="primary" style="margin-right:20px" @click="redistribute()">重新分配</Button><Button type="primary" style="margin-right:20px"  @click="distribeconfirm()">确认分配</Button><Button type="default" @click="goback()">返回</Button></Col></Row>
                  </Card>
                </Col>
...
export default {
          ...
          render: (h, data) => {
            return h(
              "span",
              {
                on: {
                  click: () => {
                    this.detialdata = data.row.detiallist;
                    this.modaltable = true;
                  }
                }
              },
              data.row.continuationSalesmanName +
                "（用户数：" +
                data.row.userNum +
                "，本次分配续入总金额：" +
  ...
  methods: {
    monthnow() {
      var now = new Date(Number(this.$route.query.endTime));
      var monthn = now.getMonth() + 1;
      var yearn = now.getFullYear();
      this.allocationTime = yearn + "年" + monthn + "月";
    },
    tablecancel() {
          ...
          that.$Modal.confirm({
            render: h => {
              return h("Input", {
                props: {
                  type: "textarea",
                  value: this.followvalue,
                  autofocus: true,
                  placeholder: "跟踪内容"
                },
                on: {
                  input: val => {
                    this.followvalue = val;
                  }
                }
              });
            },
            onOk: () => {
              let msg = {
                content: this.followvalue
              };
              automaticAllotConfirm(formData(msg)).then(resp=>{
                that.$router.go(-1);
              })
            }
          });
    ...
    getlist(data) {
      ...
      automaticAllotPreviewList(data).then(resp => {
        if (resp.status == 200 && resp.response.pagemsg.list != null) {
          let data = resp.response.pagemsg.list;
          that.agreementmsg = data;
          that.total = resp.response.pagemsg.total;
        } else {
          that.agreementmsg = [];
          that.total = 0;
        }
        that.totalmsg = resp.response.totalmsg;
