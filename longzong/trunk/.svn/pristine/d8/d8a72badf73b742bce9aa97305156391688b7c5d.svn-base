...
export default {
  ...
  methods: {
    ...
    getListPage () {
      ...
      let loginTimeStart  = formatStamp(this.serviceTime[0]);
      let loginTimeEnd = formatStamp(this.serviceTime[1])+24*60*60*1000-1000; 
      let params = {
      ...
      getUserList(formData(params)).then(res => {
        this.$Spin.hide()
        this.tableData = res.response.list.filter(item => item.loginTime && item.loginTime > 0)
