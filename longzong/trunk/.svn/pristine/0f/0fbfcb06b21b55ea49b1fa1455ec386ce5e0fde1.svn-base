    ...
    <Col span="24" class-name="search-form">
      <Form :model="formItem" :label-width="100">
        ...
        <Row type="flex" justify="space-between">
          ...
          <Col span="7">
            <FormItem label="分配时间:">
              <DatePicker v-model="distributionTime" clearable  @on-change="dateChange" type="daterange"  :options="dateType" split-panels placeholder="请选择分配时间" class="search-input"></DatePicker>
            </FormItem>
          </Col>
          <Col span="7">
            <FormItem label="用户来源:">
        ...
        <Row type="flex" justify="space-between">
           <Col span="7">
            <FormItem label="待续入时间:">
              <DatePicker
                v-model="serviceTime"
                clearable
                type="daterange"
                :options="dateType"
                placeholder="请选择待续入时间"
                class="search-input">
              </DatePicker>
            </FormItem>
          </Col>
...
export default {
  ...
  data() {
    return {
      ...
      distributionTime: [],
      customerSource: [],
      serviceTime: [new Date(new Date(new Date().toLocaleDateString()).getTime()-3600*1000*24*30),new Date(new Date(new Date().toLocaleDateString()).getTime())],
      time4:[],
      dateType: {
        shortcuts: [
          {
  ...
  methods: {
    ...
    getListPage () {
      ...
      let continueTimeStart = formatStamp(this.serviceTime[0])
      let continueTimeEnd = formatStamp(this.serviceTime[1])+24*60*60*1000-1000;
      ...
      let params = {
        ...
        continueTimeStart:continueTimeStart || '',
        continueTimeEnd:continueTimeEnd || '',
