...
import { listAdminPerformance , sumAdminPerformance,exportAdminPerformance} from "../../components/axios/produceReport.js";
...
export default {
  ...
  methods: {
    ...
    this.$Spin.show();
		listAdminPerformance(params).then(res => {
      that.total = res.response.total;
			that.tableData = res.response.list;
			that.loading = false;
    }).catch(() => {that.loading = false;});
    sumAdminPerformance(params).then(res => {
...
//部门树复选框选中
    ...
    exportReport(){
      ...
      this.$Spin.show();
      exportAdminPerformance(params).then(res=>{ 
        if(res.status==200){
