...
import { listAdminCustomer, sumCustomer,exportAdminCustomer} from "../../components/axios/produceReport.js";
...
export default {
  ...
  methods: {
    ...
    queryData() {
     ...
     this.$Spin.show();
		listAdminCustomer(params).then(res => {
    ...
    //导出
     exportReport(){
      ...
      this.$Spin.show();
      exportAdminCustomer(params).then(res=>{ 
        if(res.status==200){
