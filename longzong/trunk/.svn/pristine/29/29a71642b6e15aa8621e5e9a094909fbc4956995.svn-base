        ...
        <Row class="out-right">
          ...
          <Row>
            <FormItem label="产品分类:" v-if="item.type == 1">
              <Col span="18">
              <Cascader  class="search-input" v-model="item.productCategoryName" :data="typeList" @on-change="getproductClassify($event, index)"
          ...
          <Row class="mar-t-20">
            ...
            <FormItem label="打包价:">
              <!-- v-else  -->
              <Col span="20">
              <InputNumber v-model="item.packPrice"  element-id="inputID" @on-focus="chagne(index)" :formatter="value => `${value}元`" :parser="value => value.replace('元', '')" class="search-input"></InputNumber>
              </Col>
            </FormItem>
