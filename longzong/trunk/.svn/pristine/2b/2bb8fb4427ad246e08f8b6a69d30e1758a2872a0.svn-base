...
import { listAdminCategory,sumCategory,proCategoryList,exportAdminCategory} from "../../components/axios/produceReport.js";
...
export default {
    ...
    methods: {
		...
		var that = this;
          ...
          width:100,
          render:(h,params) => {
            return h("span",{},((this.pageNum-1)*this.pageSize) + params.index + 1)
          }
        },{
                    key:"adminName",
                    title:"姓名",
         ...
         getSumList() {
            ...
            this.$Spin.show();
            listAdminCategory(params).then(res => {
                if (res.status == 200) {
                    that.total = res.response.total;
                    let arr = res.response.list ? res.response.list : [];
                    const data = [];
                    for(let i=0; i<arr.length; i++){
                        let obj = {};
                        obj.adminName = arr[i].adminName;
                        obj.orgName = arr[i].orgName;
                        obj.parentOrgName = arr[i].parentOrgName;
                        obj.totalAmount =  arr[i].totalAmount;
                        obj.statusDesc = arr[i].statusDesc;
                        obj.jobTime = arr[i].jobTime;
...
//部门树复选框选中
    ...
    exportReport(){
      ...
      this.$Spin.show();
      exportAdminCategory(params).then(res=>{ 
        if(res.status==200){
