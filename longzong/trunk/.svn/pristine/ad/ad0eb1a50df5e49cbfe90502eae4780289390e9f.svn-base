<style lang="less" scoped>
@import "./taskplant.less";
/*
任务分配与委派
*/
</style>
<template>
  <Row>
    <Col span="24">
    <Row class-name="current-position">
      当前位置：
      <router-link to="/plantIndex" class="link-css"> 信息发布管理</router-link>                      
      &nbsp;&nbsp;>>&nbsp;&nbsp;
      <router-link to="/taskPlant" class="link-css"> 任务分配与委派</router-link>                      
    </Row>
    </Col>
    <Col span="24">
    <Row :gutter="24">
      <Col span="4">
      <div class="plant-title">
        <p>人员信息列表</p>
      </div>
      <Row class="plant-content">
        <Tree
          :data="treeData"
          :load-data="findSon"
          @on-select-change='findlist'
        ></Tree>
      </Row>
      </Col>
      <Col span="20">
      <Row>
        <Col
          span="24"
          class-name="search-form"
        >
        <Form
          :model="form"
          :label-width="80"
        >
          <Row
            type="flex"
            justify="space-between"
          >
            <Col span="7">
            <FormItem label="姓名:">
              <Input
                v-model="form.name"
                class="search-input"
                placeholder="请输入姓名/拼音"
              ></Input>
            </FormItem>
            </Col>
            <Col span="7">
            <FormItem label="状态:">
              <Select
                v-model="form.job"
                class="search-input"
                placeholder=""
              >
                <Option
                  v-for="item in jobList"
                  :value="item.value"
                  :key="item.value"
                >{{ item.statusDes }}</Option>
              </Select>
            </FormItem>
            </Col>
            <Col
              span="7"
              class="text-right"
            >
            <Button
              type="primary"
              icon="ios-search"
              @click=" search"
            >查询</Button>
            <Button
              style="margin-left: 8px"
              icon="ios-refresh"
              @click="handleReset"
            >重置</Button>
            </Col>
          </Row>
        </Form>
        </Col>
        <Col
          span="24"
          class="table-con"
        >
        <Table
          class="tableCommon"
          :data="tableData"
          :columns="columns"
          ref="dragTable"
          :stripe="true"
          :border="true"
        ></Table>
        <oil-page
          :total="total"
          :page-num="pageNum"
          :page-size="pageSize"
          @page-change="pageChange"
          :page-size-opts="pageSizeOpts"
          :show-sizer="showSizer"
        ></oil-page>
        </Col>
      </Row>
      </Col>
    </Row>
    </Col>
  </Row>
</template>
<script>
import axios from "axios";
import oilPage from "../../components/page/page.vue";
import {
  departmentTree,
  departmentList
} from "../unitmangement/api/telDurationSet.js";
import { formData } from "../../../assets/js/common.js";
import { delAbnormalLogs } from "../../components/axios/performanceManagement";
export default {
  name: "taskplant",
  components: {
    oilPage
  },
  data() {
    return {
      pageSizeOpts: [10, 20, 50, 100, 200],
      showSizer: true,
      pageSize: 10,
      total: 0,
      pageNum: 1,
      form: {
        name: "",
        job: -1
      },
      jobList: [
        { value: -1, statusDes: "全部" },
        { value: 0, statusDes: "离职" },
        { value: 1, statusDes: "在职" }
      ],
      columns: [
        {
          title: "序号",
          type: "index",
          key: "id",
          align: "center"
        },
        {
          title: "姓名",
          key: "name",
          align: "center"
        },
        {
          title: "性别",
          align: "center",
          key: "sex",
          render: (h, data) => {
            let label =
              data.row.sex === 0 ? "女" : data.row.sex === 1 ? "男" : "";
            return h(
              "span",
              {
                props: {
                  type: "text",
                  size: "small"
                }
              },
              label
            );
          }
        },
        {
          title: "联系电话",
          align: "center",
          key: "mobile"
        },
        {
          title: "所在部门",
          align: "center",
          key: "depatmentName"
        },
        {
          title: "职位",
          align: "center",
          key: "rankName"
        },
        {
          key: "status",
          align: "center",
          title: "状态",
          render: (h, data) => {
            let label =
              data.row.job === 0 ? "离职" : data.row.job === 1 ? "在职" : "";
            return h(
              "span",
              {
                props: {
                  type: "text",
                  size: "small"
                }
              },
              label
            );
          }
        },
        {
          key: "operate",
          align: "center",
          title: "操作",
          render: (h, data) => {
            return h(
              "a",
              {
                props: {},
                on: {
                  click: () => {
                    this.$router.push({
                      name: "taskplantDetail",
                      query: { id: data.row.id, deptId: data.row.orgId }
                    });
                  }
                }
              },
              "分配"
            );
          }
        }
      ],
      selectCondition: {
        treeCode: "",
        treeName: "",
        order: "asc",
        pageNum: 1,
        pageSize: 10,
        parentCode: "",
        parentId: "",
        sort: "",
        status: ""
      },
      tableData: [],
      treeData: []
    };
  },
  created() {
    this.initTree();
    this.getlist();
  },
  methods: {
    //点击查询按钮
    search() {
      let that = this;
      that.selectCondition.name = that.form.name;
      that.selectCondition.status = that.form.job == -1 ? "" : that.form.job;
      this.selectCondition.pageNum = 1;
      this.selectCondition.pageSize = 10;
      that.pageNum = 1;
      that.pageSize = 10;
      that.getlist(that.selectCondition);
    },
    //初始化树的数据
    initTree() {
      let that = this;
      departmentTree().then(res => {
        that.treeData = res.response;
      });
    },

    //获取树的数据
    findSon(item, callback) {
      var that = this;
      let params = {
        code: item.code
      };
      departmentTree(formData(params)).then(res => {
        let resultdata = res.response;
        for (var i = 0; i < resultdata.length; i++) {
          resultdata[i].title = res.response[i].name;
          resultdata[i].id = res.response[i].id;
          if (resultdata[i].isLeaf == 0) {
            resultdata[i].loading = false;
            resultdata[i].children = [];
          }
        }
        callback(resultdata);
      });
    },
    //通过点击树的数据获取相应的表格
    findlist(data) {
      this.selectCondition = data[0];
      if (this.selectCondition.isClick) {
        this.getlist();
      }
    },
    //获取分页数据
    pageChange(page, pageSize) {
      if (page) {
        this.selectCondition.pageNum = page;
        this.pageNum = page;
      }
      if (pageSize) {
        this.selectCondition.pageSize = pageSize;
        this.pageSize = pageSize;
      }
      this.getlist(this.selectCondition);
    },
    append(data) {
      const children = data.children || [];
      children.push({
        title: "appended node",
        expand: true
      });
      this.$set(data, "children", children);
    },
    //获取列表数据
    getlist() {
      let that = this;
      var params = {
        pageSize: that.pageSize,
        pageNum: that.pageNum,
        name: that.form.name == undefined ? "" : that.form.name,
        job: that.form.job == -1 ? "" : that.form.job,
        departmentId:
          that.selectCondition.id == undefined ? "" : that.selectCondition.id
      };
      departmentList(formData(params)).then(res => {
        that.tableData = res.response.list;
        that.total = res.response.total;
      });
    },
    //重置
    handleReset() {
      this.form = {};
      this.form.job = -1;
    }
  }
};
</script>