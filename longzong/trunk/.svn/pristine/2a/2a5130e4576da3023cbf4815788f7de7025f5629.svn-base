<template>
    <div style="width:100%;height:100%;" id="CSMcustomAccount"></div>
</template>

<script>
import echarts from 'echarts';
export default {
    name: 'CSMcustomAccount',
    ...
    methods:{
        getechartdata(){
            this.$nextTick(() => {
                let CSMcustomAccount = echarts.init(document.getElementById('CSMcustomAccount'));
                let xAxisData = [];
                let data1 = [];
                let data2 = [];
                for (let i = 0; i < 20; i++) {
                ...
                CSMcustomAccount.setOption(option);

                window.addEventListener('resize', function () {
                    CSMcustomAccount.resize();
                });
            });
        }
    },
