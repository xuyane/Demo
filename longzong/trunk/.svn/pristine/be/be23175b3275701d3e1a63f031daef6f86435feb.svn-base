...
<template>
  <Row>
    ...
    <div class-name="table-con" style="text-align:center;margin-top:20px;">
      ...
      <Button type="default" class="search-btn" @click="returnMark">取消</Button>
    </div>
    </Col>
    <!-- 保存按钮弹窗 -->
    <Modal v-model="saveModal" title="提示信息" width="300" @on-ok="ok" @on-cancel="cancel">
      <Row>
        <p style="text-align: center;">请完成所有目标分配后再保存!</p>
      </Row>
      <div slot="footer">
        <i-button type="default" @click="saveModal=false">返回</i-button>
      </div>
    </Modal>
  </Row>
</template>
...
<script>
  import { listOrgOldSalesMonth, saveOrgOldSalesMonth } from "../../components/axios/performanceManagement.js";
  ...
  export default {
    ...
    methods: {
      ...
      //营销业绩：部门任务-续入月分摊列表
      getListOrgOldSalesMonth() {
        let form = {
          orgId: this.orgId,
          selectYear: this.selectYear
        }
        form.time=new Date().getTime();
        listOrgOldSalesMonth(form).then(res => {
      ...
      saveData() {
        ...
        if (this.validateData()) {
          ...
          saveOrgOldSalesMonth(form).then(res => {
            this.tableData.map(item => {
              item.isInput1 = false;
              item.isInput2 = false;
              item.isInput3 = false;
            ...
            const arr = new Array(...this.tableData);
            this.tableData = arr;
            // this.$router.push({ name: 'continueMouthTaskSet' })
