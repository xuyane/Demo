...
<template>
  <Col>
  <Modal v-model="showModal" title="编辑品种" @on-cancel="doCancel" width="750">
    <Form ref="addModal" :model="form" :rules="addRules" :label-width="100" class="addform">
      <FormItem label="品种类别" prop='breedType'>
        <Row>
          <Col>
          <Row>
          ...
          <Col v-if='breedBreedTypeRelations.length>0' class="hadwrap">
  ...
  <Modal v-model="showselectModal" title="选择品种类别" @on-cancel="cancel" width="900" class="selectgg">
    <Form :model="form" ref="selectformRef" class="addform" :label-width="100">
      <Row type="flex" justify="space-between">
        <Col span="7">
        <FormItem label="品种类别编号:" >
          <Input v-model="selectform.code" placeholder="请输入品种类别编号"></Input>
        </FormItem>
        </Col>
        <Col span="7">
        <FormItem label="品种类别名称:" >
          <Input v-model="selectform.breedTypeRoadName" placeholder="请输入品种类别名称"></Input>
        </FormItem>
        </Col>
        <Col span="7">
        <FormItem>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      breedTypeRoles: {
        breedType: [
          { validator: validatePassBreedType, trigger: "change" }]
      },
      ...
      addRules: {
        breedType:[
           { required: true, validator: validatePassBreedType, trigger: "blur" }
        ],
        name: [
  ...
  methods: {
    ...
    initData(id) {
      ...
      axios.post("/basedata/breed/edit_detail", params).then(function(res) {
        if (res.data.status == 200) {
          that.form = res.data.response;
          that.breedBreedTypeRelations = res.data.response.breedTypeList;
          that.tid = id;
          that.$refs.childData.getData(that.form.name);
        } else {
