  ...
  <Row>
    <Col span="24">
      <Row class-name="current-position">
          ...
          &nbsp;&nbsp;>>&nbsp;&nbsp;
          <a @click="goto" class="link-css"> 修改专栏</a>
  ...
  methods: {
    ...
    editSubmit() {
      this.$refs.addCreateTopicRef.validate(valid => {
        if (valid) {
          ...
          columnBaseUpdate(params).then(res => {
            this.$Message.success("编辑成功");
            this.$router.push({
              name: "columnList",
              query: {
                id: Number(this.$route.query.topicid)
              }
            });
          });
        }
    ...
    initDataOne() {
      ...
      columnBaseInfo(params).then(res => {
        that.addCreateTopic.columnName = res.response.columnName;
        that.addCreateTopic.columnModelId = res.response.columnModelId;
        that.addCreateTopic.columnModelName = res.response.columnModelName;
        that.queryModelList();
        that.addCreateTopic.lineNum = res.response.lineNum;
