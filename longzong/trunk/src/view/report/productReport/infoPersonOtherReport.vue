...
import { querySinglMsgOthers, singleQtExcept, list_page_single_bg} from "../../components/axios/produceReport.js";
...
export default {
  ...
  data() { 
    ...
    columns1:[
        ...
        },
        {
          ...
          width:150,
          render: (h, data) => {
            return h("span",{},Number(data.row.dePecent) == 0 ? 0 + '%' : (data.row.dePecent * 100).toFixed(2) + '%');
          }
				},
			],
		columns: [
                ...
                on:{
                  click:() =>{
                    if(Number(data.row.dzljgxd) === 0){
                      ...
                      this.pageNum1 = 1;
                      this.pageSize1 = 10;
                      this.eId = data.row.empId
                      this.eType = 0;
                      this.contributionList(data.row.empId,this.eType);
                     }
                  }
                }
                ...
                on:{
                  click:() =>{
                    if(Number(data.row.dyljgxd) === 0){
                      ...
                      this.pageNum1 = 1;
                      this.pageSize1 = 10;
                      this.contributionList(this.eId,this.eType);
                    }
  ...
  methods: {
    ...
    this.$Spin.show();
		querySinglMsgOthers(params).then(res => {
			that.tableData = res.response.reportXxqtVOList.list;
			that.total = res.response.reportXxqtVOList.total;
    ...
    initDateRange() {
     ...
     // 监听选择日期
        ...
        list_page_single_bg(from).then(res =>{
    ...
    exportReport(){
      ...
      this.$Spin.show();
      singleQtExcept(formData(params)).then(res=>{ 
        if(res.status==200){
