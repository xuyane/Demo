...
.info-detail{border-left: 1px solid #ddd;width:60%;}
</style>
<template style="display:inline-block">
  <Row style="display:inline-block">
    <Col><i-button type="primary" @click="addformationUnit()">新增信息单元</i-button></Col>
    <Modal v-model="formationUnit" title="新增信息单元" width="85%">
         <Col span="24">
        <Row class-name="add-container">
            ...
            <Form ref="formItem" :model="formItem" :rules='ruleValidate' :label-width="150"  >
                <FormItem label="信息单元名称:" prop="IsNameNull" style="margin:0 auto">
                ...
                <Row >
                    <Col span="6">
                        <FormItem label="发布周期:" prop="releaseCycle">
                            <Select  class="search-input" v-model="formItem.releaseCycle"  @on-change="changeTypeList1">
                                <Option v-for="item in typeList1" :value="item.value" :key="item.value">{{ item.label }}</Option>
                            </Select>
                        </FormItem>
                    </Col>
                    <Col span="7" v-if="formItem.releaseCycle != 0 && formItem.releaseCycle !== ''">
                        ...
                        <FormItem label="要求完成时间:" v-if="formItem.releaseCycle == 4" prop="completeTime2">
                            <DatePicker type="date" format="yyyy-MM-dd" v-model="formItem.completeTime2" placeholder="请选择日期"  style="width: 200px" @on-change="changeCompleteTime2"></DatePicker>
                        </FormItem>
                    </Col>
                    <Col span="6">
                ...
                <Row>
                    <Col >
                        <FormItem label="发布时间:" prop="releaseTime">
                            <TimePicker type="time" format="HH:mm" v-model="formItem.releaseTime" style="width: 168px"></TimePicker>
                        </FormItem>
                    </Col>
                </Row>
     ...
     <div slot="footer">
        <Button type="primary" @click="handleSubmit('formItem',1)">保存</Button>
     ...
     export default {
        ...
        data () {
            ...
            const LargeClassList = (rule, value, callback) => {
                if(this.formItem.type === ''){  //要写为 '===',因为当为数字0时会认为控制报错
            ...
            return { 
                ...
                ruleValidate: {
                    ...
                    siteOne :[
                        {  required: true, validator:validateInfoSite,message:'请输入站点',trigger:'change'}
                    ],
                    info: [
                        {   required: true, validator: validateInfo, message: '请输入关联的信息分类', trigger: 'change' }
                    ],
                    releaseCycle: [
                        {  required: true,type: 'number', message: '请选择发布周期', trigger: 'change' }
                    ],
                    completeTime1: [
                        {  required: true,type: 'number', message: '请选择要求完成时间', trigger: 'change' }
                    ],
                    completeTime2: [
                        {  required: true,validateor:validateTime, message: '请选择要求完成时间', trigger: 'change' }
                    ],
                    releaseCount: { required: true, type: 'string', message: '请输入发布次数', trigger: 'blur' },
                    releaseTime: { required: true,validator: validateReleaseTime, message: '请选择最晚发布时间点', trigger: 'change' },
                    TaskResponsibility:{ required: true,validator:TaskRes,trigger: 'change' },
                    LargeClass:{required: true,validator: LargeClassList, trigger: 'change'},
                    BigType:{required:true,validator:BigTypeIsflag,trigger:'change'},
                    defaultKeywordOne:[
                        {
                            type: "string",
                            max: 100,
        ...
        methods: {
            ...
            handleSubmit (name,idOne) {
                ...
                if (valid) {
                    console.log('xuyane');
                    const  Connect = that.formItem.InfoClass;
                    const ConVariet = that.formItem.InfoVarieties;
                    const ConArea = that.formItem.InfoArea;
                    const ConEnter = that.formItem.InfoEnterprise;
                    ...
                    // 判断周期
                    let one = 0;
                    if(that.formItem.releaseCycle==1 || that.formItem.releaseCycle==2 || that.formItem.releaseCycle== 3){
                    ...
                    console.log('one',one);
                        ...
                        this.$Spin.show();
                        let form = {
                            realtionTaskId:that.$route.query.taskId,
            ...
            addformationUnit(){
                console.log(this.formationUnit)
                this.formationUnit = true;
            ...
            reset(){
                this.formItem.type="";
                this.formItem.largeClassInfo="";
                this.formItem.productId="";
                this.formItem.name="";
                this.formItem.siteId="";
                 this.formItem.info="";
                this.formItem.isShow=0;
                this.formItem.defaultKeyword="";
                this.formItem.defaultSummary="";
                this.formItem.defaultSource="";
                this.formItem.taskCycle="";
                this.formItem.executorId="";
                this.formItem.executorName="";
                this.formItem.releaseCycle="";
                this.formItem.releaseCount="";
                this.formItem.releaseTime="";
                this.formItem.completeTime1="";
                this.formItem.completeTime2="";
                ...
                this.formItem.isHolidayShow=0;
                this.informationList0=[];
                this.informationList1=[];
                this.informationList2=[];
                this.informationList3=[];
            }
