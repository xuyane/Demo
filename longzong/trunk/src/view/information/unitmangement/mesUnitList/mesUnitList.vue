...
<template>
    <Row>
        ...
        <Row class-name="current-position">
            ...
            &nbsp;&nbsp;>>&nbsp;&nbsp; 
            <a @click="goto" class="link-css">关联指标</a>
        ...
        <to-base-information-relation-modal :showRelationModal="showRelationModal" :linkerData="linkerData" @hide-Relation-Modal="hideRelationModal"
            ref="relationIfo"></to-base-information-relation-modal>
  ...
  methods: {
    ...
    getTaskQuotaRelation() {
      ...
      getTaskQuotaRelation(formData(form)).then(res => {
        if (
          res.response.quotaVOList != null &&
          res.response.quotaVOList.length > 0
        ) {
    ...
    delete(data) {
      // this.data6.length <= 1 ? this.$Message.warning('已经最后一行了') : this.data6.splice(data.index,1);
      //如果是最后一行，删除数据后要添加一条空数据
      if (this.data6.length <= 1) {
