  ...
  <Row>
    <Form :label-width="150">
         <FormItem label="产品:" >
            ...
            <Button type="primary" @click="infoProduct()">关联产品</Button>
            <Button type="primary" v-if="productName!=''" @click="noInfoProduct()" >取消关联</Button>
        </FormItem>
    </Form>
    
<!-- 关联模板开始 -->
    <Modal v-model="productMadel" title="关联产品" width="900">
...
import { productList  } from "./api/unitManageMent";
export default {
  components: {
    oilPage
  },
              ...
              return h('Checkbox', {
                    ...
                    on: {
                        'on-change': (e) => {
                            ...
                            // console.log(this.tableData_info[params.index]);
  ...
  methods: {
    ...
    infoProduct(data) {
    if (this.siteChangeNum === -1) {
        return this.$emit('connectProduct2', false)
      }
      this.productMadel = true;
      this.pageNum = 1;
      this.tableData_info=[];
      this.getListPage();
    },
    ...
    doEdit() {
      ...
      that.loading1 = true;
        ...
        if(that.selectRow.infomationName == '' ||that.selectRow.infomationName ==null ){
            ...
            that.productMadel = false;
            that.loading1 = false;
        }else{
          that.$Message.warning('已关联信息单元不能被选择');
          console.log("已关联信息单元非空校验");
          console.log(that.selectRow);
