    ...
    <Modal v-model="AssoInforClass" title="关联信息分类" width="900">
      <Form ref="shenheform" :model="Assonform" :label-width="110">
          <Row>
              <!-- <Col span="11">
              <FormItem label="站点">
                  <Input :value="this.siteChange.siteName" disabled id="inp"></Input>
              </FormItem>   
              </Col> -->
...
import { informationtypeList, informationType,editDetail } from "./api/unitManageMent";
export default {
  name: "selectConnect",
  components: {
    oilPage
  ...
  created() {
    console.log("12123111111111111111",this.informationList);
    this.getEditDetia();
    
    this.isFlag = this.istrue;
    this.siteChangeNum = this.siteChange.siteId;
  },
  methods: {
    getEditDetia(){
      let form={
    ...
    xuanzheInfo(data) {
      if (this.siteChangeNum === -1 && this.isFlag) {
    ...
    doEdit() {
      ...
      if (that.selectRow.length > 0) {
        for (let i = 0; i < that.selectRow.length; i++) {
          ...
          breedObj.name = that.selectRow[i].informationTypeRoadName;
  ...
  watch: {
    siteChange(newV, oldV) {
      ...
      this.Assonform.siteId = newV.siteId;
      this.options1 = [];
