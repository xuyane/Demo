<template>
    <div style="width:100%;height:100%;" id="GMcustomAccount"></div>
</template>

<script>
import echarts from 'echarts';
export default {
    name: 'GMcustomAccount',
    data () {
        return {
            //
        };
    ...
    mounted () {
        this.$nextTick(() => {
            let GMcustomAccount = echarts.init(document.getElementById('GMcustomAccount'));
            let xAxisData = [];
            let data1 = [];
            let data2 = [];
            for (let i = 0; i < 20; i++) {
            ...
            GMcustomAccount.setOption(option);

            window.addEventListener('resize', function () {
                GMcustomAccount.resize();
            });
        });
    }
};
