<template>
    <div style="width:100%;height:100%;" id="McustomAccount"></div>
</template>
...
export default {
    ...
    methods:{
        getechartdata(){
            this.$nextTick(() => {
                let McustomAccount = echarts.init(document.getElementById('McustomAccount'));
                let xAxisData = [];
                let data1 = [];
                let data2 = [];
                for (let i = 0; i < 20; i++) {
                ...
                McustomAccount.setOption(option);

                window.addEventListener('resize', function () {
                    McustomAccount.resize();
                });
            });
        }
    },
