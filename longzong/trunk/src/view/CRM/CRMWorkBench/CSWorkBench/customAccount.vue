<template>
    <div style="width:100%;height:100%;" id="CScustomAccount"></div>
</template>

<script>
import echarts from 'echarts';
export default {
    name: 'CScustomAccount',
    ...
    methods:{
        getechartdata(){
            this.$nextTick(() => {
            let CScustomAccount = echarts.init(document.getElementById('CScustomAccount'));
            let xAxisData = [];
            let data1 = [];
            let data2 = [];
            for (let i = 0; i < 20; i++) {
            ...
            CScustomAccount.setOption(option);

            window.addEventListener('resize', function () {
                CScustomAccount.resize();
            });
        });
        }
    },
