<template>
    <div style="width:100%;height:100%;" id="MDcustomAccount"></div>
</template>

<script>
import echarts from 'echarts';
export default {
    name: 'MDcustomAccount',
    data () {
        return {
            //
        };
    ...
    mounted () {
        this.$nextTick(() => {
            let MDcustomAccount = echarts.init(document.getElementById('MDcustomAccount'));
            let xAxisData = [];
            let data1 = [];
            let data2 = [];
            for (let i = 0; i < 20; i++) {
            ...
            MDcustomAccount.setOption(option);

            window.addEventListener('resize', function () {
                MDcustomAccount.resize();
            });
        });
    }
};
