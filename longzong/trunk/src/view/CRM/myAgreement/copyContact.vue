          ...
          <table width="100%" border="0" cellspacing="0" class="tablewrap">
            <Row>
              <Col span="12" class="row" >
                ...
                <td class="td blueword1 memberA" v-if="isclick">
                  <Input type="text" style="width:60%;" v-model="firstPartName" size="small" @on-change="gsblur"></Input>
                </td>
                <td class="td blueword1 memberA" v-else>
                  <associate-Search  @query-list="companyTypeFunction" :dataUrl="url" :parameterName='name' class='companyipt smallipt' ref="firstPart"></associate-Search>
                </td>
...
export default {
                    ...
                    h("InputNumber", {
                      ...
                      on: {
                        ...
                        "on-change": num => {
                          ...
                          this.productList[data.index].subtotalStandardPrice = (
                            Number(data.row.afterDiscountPrice).toFixed(2) *
                            100 *
                            this.productList[data.index].ubList.length *
                            num /
                            100
            ...
            return h("Input", {
              ...
              on: {
                "on-change": event => {
                  ...
                  this.productList[data.index].description = value;
                }
              }
            });
          }
  ...
  methods: {
    ...
    getMsg() {
      ...
      axios
        ...
        .then(function(response) {
          ...
          that.$refs.breedDataRef.query = response.data.response.userName;

          that.gridColumns.Representative_a = response.data.response.delegateA;
          that.gridColumns.tel_a = response.data.response.phoneA;
          that.effectiveType = String(response.data.response.effectiveType);
          // that.time=formatTime2(response.data.response.effectiveTime)
          that.isNeedInvoice = String(response.data.response.isNeedInvoice);
        })
    ...
    discountitem(item) {
      ...
      if (type == 1) {
        this.totalAmount = this.totalAmount * item.ob.minRebate;
      } else if (type == 2) {
        // let allselect = 0;
    ...
    getdiscount() {
      ...
      contractSaleStrategy(list).then(resp => {
        if (
          ...
          resp.response.rebateVOs.length == 0
        ) {
        } else {
    ...
    savrorsendcontact(a, b) {
      ...
      saveAgreement(data).then(resp => {
        ...
        if (b == 0) {
          ...
          // that.$router.go(-1);
          that.$router.push({ name: "myAgreement" });
        } else {
          that.$router.go(-1);
        }
      });
