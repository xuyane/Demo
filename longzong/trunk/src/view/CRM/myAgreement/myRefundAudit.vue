...
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          key: "createTime",
          render:(h,data)=>{
           return h('span',{},data.row.createTime?timeStampChange.format(data.row.createTime):data.row.createTime)
         }
        },
        {
          title: "认账日期",
          ...
          key: "accountTime",
          render:(h,data)=>{
           return h('span',{},data.row.accountTime?timeStampChange.format(data.row.accountTime):data.row.accountTime)
         }
        },
        {
          title: "业务员",
  ...
  methods: {
    ...
    getlist(data) {
      ...
      //   delete data.adminDeptValue
      // }
      contractChangeapplyMyList(formData(data)).then(resp => {
        that.spinShow=false;
        if (resp.status == 200) {
          if(resp.response){
