  ...
  <Row>
    ...
    <Col span="24" class-name="card-container">
      <Col>
        ...
        <span><Button class="search-btn-w" @click="back">取消合并</Button></span>
      </Col>
        ...
        <Col span="8" style="padding:0 20px">
          ...
          <transition-group tag="ul" name="move">
            <li class="box-merge" v-for="(item, index) in operationCompany" :key="index" v-if="index > 0">
              <div class="header-name text-center" :ref="'table' + index">{{item.memberNameCn}}
                ...
                <div class="flex flex-bewteen">
                  ...
                  <div>
                    <button v-show="!item.areaCodeState" class="set-btn-w"
                      @click="setStandardOne(item.postcode, 'areaCodeState', index)">设为标准</button>
        ...
        <Col span="8" style="padding:0 20px" v-if="operationCompany.length">
          ...
          <div class="box-merge">
            ...
            <div class="content-item">
              ...
              <div class="flex flex-bewteen">
                <div>邮政编码: {{operationCompany[0].postcode}}</div>
                <div>
                  <button v-show="!operationCompany[0].areaCodeState" class="set-btn-w"
                    @click="setStandardOne(operationCompany[0].postcode, 'areaCodeState', 0)">设为标准</button>
...
export default {
  ...
  methods: {
    getListPage () {
      ...
      this.$Spin.show()
      getOrdinaryMergeDetail(formData(params)).then(res => {
        ...
        if(i === index) {
          ...
          switch(str) {
            ...
            case 'addressState':
              ...
              break;
            case 'areaCodeState':
              self.resultCompany.postcode = value
              break;
          }
        } else {
          ele[str] = false
    ...
    confirm() {
      ...
      let params = {
        ...
        postcode: this.resultCompany.postcode || '',
