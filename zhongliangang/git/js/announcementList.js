        ...
        laypage.render({
            ...
            jump: function (obj, first) {
                ...
                if (!first) {
                    ...
                    pageSize = obj.limit
                    pageNum = obj.curr
                    getData(categoryID,'1',cname,putdate,obj.curr,pageSize,false)
                }
            }
        });
    });
...
function drawingList(data){
    ...
    for(var i =0;i<data.length;i++){
        str += '<tr><td>'+(pageSize*(pageNum-1)+ i+1)+'</td><td><a href="###" target="_blank" title="'+data[i].title+'">'+data[i].title+'</a> </td><td>'+data[i].cname+'</td><td>'+data[i].putdate+'</td></tr>';
    }
