...
try {
    $.ajax({
        type: "get",
        url: "http://cgmn.custeel.com/cgnews.mv?method=getEcnews&str=8,1001001",
        dataType: "jsonp",
        jsonp: 'callback',
        success: function (data) {
            ...
            for (var i = 0; i < arr.length; i++) {
                ...
                if (arr[i].viewUser === 0) {
                    // 跳转详情页面
                    str += '<li><a href="http://ec.custeel.com/client/news/newsBrowse.jsp?timeNumber=' + arr[i].number + '" target="_blank" title="' + arr[i].title + '">' + arr[i].title + '</a><span>'+resetTime(arr[i].pushdate)+'</span></li>'
                } else {
                    str += '<li><a href="http://ec.custeel.com/home/member/login.html" target="_blank" title="' + arr[i].title + '">' + arr[i].title + '</a><span>'+resetTime(arr[i].pushdate)+'</span></li>'
                }
            }
