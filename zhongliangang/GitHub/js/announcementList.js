    ...
    function getData(categoryID, title, cname, putdate, pageNum, pageSize, ispage) {
        ...
        $.ajax({
            type: "get",
            url: baseUrl+"/cgnews.mv?method=getCgnewAlls",
            data: {
            ...
            for (var i = 0; i < data.length; i++) {
                if (data[i].level == 0) {
                    str += '<a href="./announcementDetail.html?number=' + data[i].number + '"target="_blank"  title="' + data[i].title + '">' +
                        ...
                        '<span>' + resetData(data[i].putdate) + '</span>' +
                        '</a>'
                } else {
                    str += '<a href="./login.html" target="_blank" title="' + data[i].title + '">' +
                        '<span>' + (pageSize * (pageNum - 1) + i + 1) + '</span>' +
                        ' <span>' + data[i].title + '</span>' +
                        '<span>' + data[i].cname + '</span>' +
                        '<span>' + resetData(data[i].putdate) + '</span>' +
    ...
    $(function(){
        if(window.location.search.indexOf('categoryID')>-1){
            ...
            $('.typeList ul li').each(function(i,v){
                if($(v).attr('data-val')==categoryID){
