...
$('.noticeList').on('click', 'table>tbody>tr', function () {
    ...
    if (_this.html().indexOf('underway') > -1) {
        ...
        if(dataArr[index].apMode==0){
            ...
            '(此标书为邀请投标，请 &nbsp;&nbsp; <a class="more" href="./login.html" target="_blank">登录</a>&nbsp;&nbsp;查看 )' +
            '</td></tr>';
        }else{
            // 如果是公开标书
            $.ajax({
                type: "get",
                url: baseUrl+"/cgnews.mv?method=getbidsDetail",
                data: {
                ...
                success: function (data) {
                    ...
                    if(data.data.length>0){
                        ...
                        '<tr>' +
                        '<td class="explain" colspan="6">' +
                        '(更多规格详情，请 &nbsp;&nbsp; <a class="more" href="./login.html" target="_blank">登录</a>&nbsp;&nbsp;查看 )' +
                        '</td></tr>';
                    }
...
$('.noticeList').on('dblclick', 'table>tbody>tr', function () {
    ...
    // 只针对ie9以上有效
    var a = $("<a href='./login.html' target='_blank'>0.0</a>").get(0);
...
function drawingList(data) {
    ...
    if(data.length>0){
        for (var i = 0; i < data.length; i++) {
            ...
            str += '<tr><td>' + (pageSize * (pageNum - 1) + i + 1) + '</td><td><a href="javascript:;" title="' + data[i].bname + '">' + data[i].bname + '</a> </td><td>' + data[i].numberShow + '</td>' + status + '<td>' + data[i].beTime + '</td></tr>';
        }
    }else{
...
function getData(tname, cname, putdate, pageNum, pageSize, ispage) {
    ...
    $.ajax({
        type: "get",
        url: baseUrl+"/cgnews.mv?method=getBidsAlls",
        data: {
            tname: tname,
            cname: cname,
            putdate: putdate,
...
// 左侧点击公开标书
$('.wrap .left .typeList ul li').eq(1).on('click',function(){
    pageNum=1;
    pageSize=20;
    putdate='';
    tname='';
    ...
    $.ajax({
        type: "get",
        url: baseUrl+"/cgnews.mv?method=getBidsAlls&apmode=1",
        data: {
            pageNum: pageNum,
            pageSize: pageSize
        },
...
$.ajax({
    type: "get",
    url: baseUrl+"/cgnews.mv?method=getBidsAlls&apmode=1&pageSize=5",
    dataType: "jsonp",
    jsonp: 'callback',
    success: function (data) {
        // console.log(data)
        ...
        for (var i = 0; i < newData.length; i++) {
            str += '<li><a href="./login.html" target="_blank">' + newData[i].bname + '</a></li>';
        }
        str += '<li>快来参与吧！</li>';
