...
function getchart(char_dom,begintime,title,obj,fun) {
  ...
  function wymZstChart(){
	...
	$.ajax({
        type: "get",  
        url: baseUrl+"/cgnews.mv?method=getCgCompnyAlls",
         dataType:"jsonp",   
         jsonp:'callback',  
        success:function(data) {  
	...
	try{
		$.ajax({
			type: "get",  
			url: baseUrl+"/cgnews.mv?method=getEcnews&str=7,1002001",
			 dataType:"jsonp",   
			 jsonp:'callback',  
			success:function(data) {  
				...
				for(var i = 0;i<arr.length;i++){
					...
					if(arr[i].viewUser==0){
						...
						// 跳转登录界面
						str +='<a href="./login.html" target="_blank" title="'+arr[i].title+'"><span>'+(i+1)+'</span>'+
									'<span>'+arr[i].title+'</span>'+
									'<span>'+arr[i].pushdate+'</span>'+
								'</a>';
					}
	...
	try{
		$.ajax({
			type: "get",  
			url: baseUrl+"/cgnews.mv?method=getEcnews&str=7,1002001,1002003,1002004,1002005",
			 dataType:"jsonp",   
			 jsonp:'callback',  
			success:function(data) {  
	...
	try{
		$.ajax({
			type: "get",  
			url: baseUrl+"/cgnews.mv?method=getEcnews&str=7,1002002",
			 dataType:"jsonp",   
			 jsonp:'callback',  
			success:function(data) {  
	...
	try{
		$.ajax({
			type: "get",  
			url: baseUrl+"/cgnews.mv?method=getCgbids&status=0&rows=13",
			 dataType:"jsonp",   
			 jsonp:'callback',  
			success:function(data) {  
				...
				for(var i = 0;i<arr.length;i++){
					// console.log('arr[i].viewUser',arr[i].viewUser)
					// 跳转详情页面
					str += '<a href="./login.html" target="_blank" title="'+arr[i].eqname+'"><span>'+arr[i].eqname+'</span>'+
