...
$.ajax({
    type: "get",  
    url: baseUrl+"/cgnews.mv?method=getCgxsSearch",
     dataType:"jsonp",   
     jsonp:'callback',  
...
function getListData(ename,mtial,normtness,bwidth,stlcst,pageNum,pageSize,isPage){
    ...
    $('.dataList .listBody ul').prepend('<li><p style="text-align:center;padding-top:20px">数据加载中.....</p></li>');
    $.ajax({
        type: "get",  
        url: baseUrl+"/cgnews.mv?method=getCgxs",
         dataType:"jsonp", 
         data:{
...
function renderList(data){
    ...
    if(data.length>0){
        ...
        $('.dataList .listBody ul').empty();
        $('.dataList .listBody ul').prepend('<li><p style="text-align:center;padding-top:20px">暂无数据</p></li>');
    }
